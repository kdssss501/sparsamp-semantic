...
\begin{table}
\centering
\caption{Cross-precision replay results for Qwen2.5-1.5B-Instruct. Continuous intervals use 10,000 prompt-cluster bootstrap resamples. Exact recovery counts retain all trials.}
\label{tab:1}
\scriptsize
...
\begin{table}
\centering
\caption{Matched referenced-package replay controls on the 20-prompt seed-0 bundle. All byte counts include the same 101-byte referenced header and per-trial identity records.}
\label{tab:2}
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\renewcommand{\arraystretch}{1.18}
...
\begin{table}
\centering
\caption{Prespecified one-factor-at-a-time parameter sensitivity. Each row contains the same 20 prompts and public seed 0. Delta intervals use 10,000 prompt-paired bootstrap resamples. Referenced bytes include the common 101-byte header and per-trial identity records.}
\label{tab:3}
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\renewcommand{\arraystretch}{1.18}
