...
| C4 | Sparse records are substantially smaller than full token traces. | R044 mean sparse/full ratio 2.8808%, cluster 95% CI 2.3960-3.3571%. | Supported | Ratio excludes the shared configuration header and uses fixed-width step/token accounting. |
...
| C7 | Top-four reduces semantic quality. | Structural sentence completion 16/20 versus 20/20, paired difference -0.20 [-0.40, -0.05]. | Needs evidence | Sentence punctuation is not semantic quality. Requires multi-seed blinded human evaluation. |
...
| C9 | Certificates generalize to unseen hardware, kernels or model revisions. | No independent hardware experiment. | Needs evidence | Run the frozen top-two protocol on a second GPU/CUDA stack. |
