...
- 平均稀疏记录/完整 token trace 比例为 2.88%，95% CI 为 2.40%-3.36%。
- 58/60 达到公开句末结构条件。
...
1. 完成独立硬件复验。
2. 生成 Figure 1-4 的正式矢量图和 source data。
3. 完成 native/top-2/top-4 的盲法语义评价。
4. 根据目标期刊压缩摘要、主文长度和 Methods 位置。
5. 使用 Nature reviewer skill 进行一次拒稿风险审查，再进入语言 polishing。
