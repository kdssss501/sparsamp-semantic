...
`top_k=64` 时没有 q 达到门槛。`q=1/2` 的支持一致为 26/32，但完整 counts
仅 6/32；bin-mass 近似本身的平均 KL/TV 仅约 `2.55e-5 / 6.95e-4`。
因此整数质量构造作为独立组件通过偏差审计，但 64 个 bins 的联合一致性失败。
结果定性为 **PARTIAL-GO-COMPONENT / NO-GO-k64**。
