...
严格正确性定义、精确有理数反例和定理边界见
[`verified_rrc_theory.md`](verified_rrc_theory.md)。
...
论文声称的分布匹配成立于每一步使用目标分布和理想连续独立偏移的条件下。本项目默认使用
top-p=0.95，因此只对截断并量化后的实现分布 R 讨论 RRC 采样性质。未量化条件分布 Q 相对
完整模型分布 P 的**反向 KL**为 `KL(Q||P)=-log(source_mass)`；论文采用的正向
`KL(P||Q)` 在 source mass 小于 1 时为无穷。若把同配置 top-p 正常生成 Q 作为 cover，量化
偏差应报告 `KL(Q||R)`，不能写成对完整 P 的零 KL。
...
EMBED(bits, key):
  ...
  for t = 0 .. max_tokens-1:
    ...
    [L, R) <- interval(i)
    append token i
    if (L + R)/2 - ds is in (-0.5, 0.5]:
      paper mode: stop
      verified mode: stop only if full EXTRACT replay returns original ds
...
负数余数破坏反向旋转。若在 `max_tokens` 内未满足所选终止条件，返回带完整 token 前缀和
...
因此 `termination_mode="paper"` 只用于复现和反例审计，不能作为可靠通信实现。默认的
`termination_mode="verified"` 会在发送端对候选终止点执行完整反向重放，仅在接收端的
公开解码规则确实恢复原整数时停止。该检查不改变每步 token 选择，只可能增加尾部 token；
其长度侧信道和容量损失需要单独测量。逐步 token 分布匹配不保证数据相关停止后的变长全文
分布匹配；严格安全实验应使用公开固定总长度，并在嵌入完成后继续按 cover 分布采样。

500 个 Mock 审计样本中，Verified-RRC 为 500/500 精确恢复，平均只比论文模式增加
0.04-0.15 token，最大增加 2 tokens。该结果支持进入真实语义模型 smoke，但不能代替
Qwen/GPT-2 上的浮点重放、Token Ambiguity 和 steganalysis 检验。
...
- 人工分布和 MockProvider：保留可复现的 modular-wrap 反例；Verified-RRC 要求
  1/13/64/128 bit 精确 round-trip，并运行多 seed 审计。
...
- Qwen：与 Fixed-16、Fixed-64 在相同 payload、prompt、top-p、temperature 和 token budget
  下比较完成率、bits/token、token/s、Token Ambiguity、现有反向截断 KL，并新增同配置 cover
  到量化实现的论文方向 `KL(Q||R)`。
